...
if data == 'ring':
    modes = [ ((np.cos(np.pi*x), np.sin(np.pi*x)),(0.1,0.1)) for x in np.linspace(0,2,8, endpoint=False)]
    weights = np.ones(len(modes))
elif data == 'spiral':
    modes = [ ((0.3*x*np.cos(np.pi*x), 0.3*x*np.sin(np.pi*x)),(0.03,0.03)) for x in np.linspace(0,4.788,12, endpoint=True)]
...
#for _ in tf.trainable_variables(): print _
